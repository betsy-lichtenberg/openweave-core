...
extern "C" {
    ...
    NL_DLL_EXPORT void Java_nl_Weave_DeviceManager_WeaveDeviceManager_beginHush(JNIEnv *env, jobject self, jlong deviceMgrPtr, jlong hushCode, jlong challenge, jint keyId, jbyteArray key);
...
static void HandleHushOperationComplete(WeaveDeviceManager *deviceMgr, void *reqState, uint8_t hushResult, uint8_t length, const uint8_t *conditions);
...
void Java_nl_Weave_DeviceManager_WeaveDeviceManager_beginHush(JNIEnv *env, jobject self, jlong deviceMgrPtr, jlong hushCode, jlong challenge, jint keyId, jbyteArray key)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    WeaveDeviceManager *deviceMgr = (WeaveDeviceManager *)deviceMgrPtr;
    ...
    WeaveLogProgress(DeviceManager, "beginHush() called");

    keyLen = env->GetArrayLength(key);
    keyBuf = (const uint8_t *)env->GetByteArrayElements(key, NULL);

    pthread_mutex_lock(&sStackLock);

    err = deviceMgr->SignedHush(static_cast<uint32_t>(hushCode), static_cast<uint32_t>(challenge), static_cast<uint16_t>(keyId),
                keyBuf, static_cast<uint16_t>(keyLen), (void *)"Hush", HandleHushOperationComplete, HandleError);

    pthread_mutex_unlock(&sStackLock);

    if (keyBuf != NULL)
        env->ReleaseByteArrayElements(key, (jbyte *)keyBuf, JNI_ABORT);
...
void HandleHushOperationComplete(WeaveDeviceManager *deviceMgr, void *reqState, uint8_t hushResult, uint8_t length, const uint8_t *conditions)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    JNIEnv *env;
    jobject self = (jobject)deviceMgr->AppState;
    ...
    jmethodID method;
    bool localFramePushed = false;
    jbyteArray conditionsObj;

    WeaveLogProgress(DeviceManager, "Received response to Hush request", (const char *)reqState);
    ...
    err = N2J_ByteArray(env, conditions, length, conditionsObj);
    SuccessOrExit(err);

    deviceMgrCls = env->GetObjectClass(self);
    VerifyOrExit(deviceMgrCls != NULL, err = WDM_JNI_ERROR_TYPE_NOT_FOUND);

    method = env->GetMethodID(deviceMgrCls, "onHushComplete", "(B[B)V");
    VerifyOrExit(method != NULL, err = WDM_JNI_ERROR_METHOD_NOT_FOUND);

    WeaveLogProgress(DeviceManager, "Calling Java onHushComplete method");

    env->ExceptionClear();
    env->CallVoidMethod(self, method, (jbyte)hushResult, conditionsObj);
    VerifyOrExit(!env->ExceptionCheck(), err = WDM_JNI_ERROR_EXCEPTION_THROWN);
